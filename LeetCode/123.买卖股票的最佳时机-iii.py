...
class Solution(object):
    def maxProfit(self, prices):
        ...
        # dp[2][0] 初始化为 -prices[0] 是为了统一状态转移逻辑。虽然从语义上第二次买入需要先完成第一次交易，但在 DP 中我们允许状态“提前存在”，因为 dp 表示的是最优子结构，而不是严格的操作顺序
        dp = [[0, 0, 0, 0] for _ in range(len(prices))]
        dp[0][0] = 0 - prices[0]
        dp[0][1] = 0 
        dp[0][2] = 0 - prices[0] # dp只表示最优子结构，所以这里也要初始化为-prices[0], 表示买入股票
        dp[0][3] = 0 
