...
public:
    int subarraySum(vector<int>& nums, int k) {
        // 这道题比较复杂，思路很难想，涉及到数组的前缀和这个概念
        // 对于数组中的任何位置 j，前缀和 pre[j] 是数组中从第一个元素到第 j 个元素的总和。
        // 这意味着=从元素 i+1 到 j 的子数组的和，就是 pre[j] - pre[i] 的值
        int count = 0;
        int n = nums.size();
        unordered_map<int, int> map;
        map[0] = 1; // 这个是默认放进去的，因为什么也没加，前缀和为0的序列有1条
        int pre = 0;
        ...
        for(int i = 0; i < n; ++i){
            ...
            map[pre]++; 
            // 根据新的pre值，对map来说，如果map[pre]存在，就自增
            // 如果不存在，就新增一个key为pre，val为1
        }
